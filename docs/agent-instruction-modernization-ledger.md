...
- Owner/PR values of `TBD` are deliberate placeholders. This ledger never
...
| ID | Finding and disposition | Owner / PR | Protected boundary | Dependency / status | Required evidence |
|---|---|---|---|---|---|
| O-01 | Two Proposed ADRs used number 009. Renumber the Nomad-deferral proposal to ADR-021 and update its live references; retain NATS authentication as ADR-009. | Odysseus / current Wave 0 PR TBD | Do not edit Accepted ADR bodies. | Implemented in the Wave 0 worktree; review and validation pending. | Tracked/hidden reference search, ADR status check, Markdown validation, and PR diff. |
| O-02 | Add Proposed ADR-022, “Layered, Provider-Neutral Agent Instructions,” referencing ADR-020 and existing mesh/task schemas rather than duplicating them. | Odysseus / current Wave 0 PR TBD | Proposal only; dependent PRs wait for human acceptance. | Implemented in the Wave 0 worktree; acceptance pending. | Human decision plus exact accepted revision before downstream work starts. |
| O-03 | Repair incomplete or inaccurate ADR indexes and current-versus-proposed claims. Keep evidence integrity as direct repository policy without representing Proposed ADR-014 as Accepted. | Odysseus / current Wave 0 PR TBD | Accepted ADRs stay byte-unchanged. | Implemented in the Wave 0 worktree; review and validation pending. | Header-to-index comparison, protected-file diff, link and Markdown checks. |
...
| O-14 | Canonical schema/source comments still include proposal-era wording such as “M0 ratification.” Do not rewrite the `hi/v1` contract during Wave 0. Classify descriptive-only corrections with the schema owner after ADR-022 acceptance, without changing fields or compatibility. | Odysseus and schema consumers / TBD | `hi/v1` fields and canonical schema behavior are preserved; coordinated review required. | Deferred and explicitly unresolved. | Schema diff proves descriptions only, plus parser/consumer compatibility tests and owner approval. |
